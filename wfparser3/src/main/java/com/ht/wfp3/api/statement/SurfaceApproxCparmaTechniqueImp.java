...
class SurfaceApproxCparmaTechniqueImp extends SurfaceApproxImp
  ...
  SurfaceApproxCparmaTechniqueImp(BigDecimal resolutionForUAxis, BigDecimal resolutionForVAxis) {
    ...
    if (null == resolutionForUAxis) {
      throw new NullPointerException("resolutionForUAxis cannot be null");
    }
    if (null == resolutionForVAxis) {
      throw new NullPointerException("resolutionForVAxis cannot be null");
    }
    if (MINIMUM_RESOLUTION_FOR_U_AXIS.compareTo(resolutionForUAxis) > 0) {
      throw new IllegalArgumentException(
          "resolutionForUAxis must be greater or equal to " + MINIMUM_RESOLUTION_FOR_U_AXIS);
    }
    if (MINIMUM_RESOLUTION_FOR_V_AXIS.compareTo(resolutionForVAxis) > 0) {
      throw new IllegalArgumentException(
          "resolutionForVAxis must be greater or equal to " + MINIMUM_RESOLUTION_FOR_V_AXIS);
    }
  ...
  @Override
  public int compareTo(SurfaceApproxCparmaTechnique o) {
    int compareTo = resolutionForUAxis.compareTo(o.getResolutionForUAxis());
    if (0 == compareTo) {
      compareTo = resolutionForVAxis.compareTo(o.getResolutionForVAxis());
    }
    return compareTo;
  }
